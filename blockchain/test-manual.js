...
async function testBlockchain() {
        ...
        return [
          {
            ...
            totalValue: ethers.utils.parseEther("10").toString(),
            totalSupply: 10000,
            tokenPrice: ethers.utils.parseEther("0.001").toString(),
            isActive: true
          },
          {
            name: "Gold Reserve", 
            assetType: "gold",
            totalValue: ethers.utils.parseEther("5").toString(),
            totalSupply: 5000,
            tokenPrice: ethers.utils.parseEther("0.001").toString(),
            isActive: true
          }
  ...
  try {
    ...
    console.log('✅ Parse Ether Test:', ethers.utils.parseEther("1").toString());
  } catch (error) {
    console.log('❌ Ethers.js Error:', error.message);
  }
  
  // 5. Test Contract Artifact
  console.log('\n5. Testing Contract Artifacts...');
  try {
    const fs = require('fs');
    const artifactPath = './artifacts/contracts/AssetToken.sol/AssetToken.json';
    if (fs.existsSync(artifactPath)) {
      const artifact = JSON.parse(fs.readFileSync(artifactPath, 'utf8'));
      console.log('✅ Contract Artifact Found -', artifact.contractName);
